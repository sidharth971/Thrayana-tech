          ...
          {faqs.map((faq, index) => {
            ...
            return (
              <AnimatedSection key={index} delay={index * 0.05} direction="up">
                <motion.div 
                  className={`bg-card border transition-all duration-500 rounded-2xl overflow-hidden ${
                    isOpen ? "border-primary/50 shadow-[0_0_30px_rgba(212,175,55,0.15)]" : "border-white/5 hover:border-primary/30"
                  }`}
                >
                  <button
                  ...
                  >
                    <span className={`text-lg md:text-xl font-bold transition-colors duration-300 ${isOpen ? "text-primary tracking-wide" : "text-white"}`}>
                      {faq.question}
                    </span>
                    <motion.div
                      ...
                      className={`flex-shrink-0 w-8 h-8 rounded-full flex items-center justify-center transition-colors duration-300 ${
                        isOpen ? "bg-primary/20 text-primary" : "bg-secondary text-white/50 group-hover:text-white"
                      }`}
                    >
                  ...
                  <AnimatePresence>
                    {isOpen && (
                      ...
                      >
                        <div className="px-6 pb-6 md:px-8 pt-0 text-white/70 font-light text-base md:text-lg leading-relaxed">
